...
ExpressionPtr Parser::SimpleExpression()
{
	...
	{
		auto ident = IPLMakeSharePtr<IdentifierExpression>(Prev().Lexeme);
		ident->SetLocation(location.Line, location.Column);
        return ident;
	}
	else if (auto al = ArrayLiteral())
	{
		al->SetLocation(location.Line, location.Column);
		return al;
	}
	else if (auto p = ParenthesizedExpression())
	{
		p->SetLocation(location.Line, location.Column);
		return p;
	}
	else
	{
