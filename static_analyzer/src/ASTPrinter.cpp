...
ASTPrinter::ASTPrinter(std::ostringstream& os)
	: os(os)
	, nest_level(0)
	, indent_next_accept(true)
{}

void PrepNextElem(std::ostringstream& os, bool isLast)
{
	if (!isLast)
	{
		os << ",";
...
void Print(std::ostringstream& os, ASTPrinter &printer, const char* name, const ExpressionPtr& expr, bool isLast)
{
	if(expr != nullptr)
	{
...
void Print(std::ostringstream& os, ASTPrinter &printer, const char* name, const IPLVector<ExpressionPtr>& list, bool isLast)
{
	printer.InsertIndent();
	os << "\"" << name << "\": [";
	if (list.empty())
...
void Print(std::ostringstream& os, ASTPrinter& printer, const char* name, const IPLVector<IPLString>& list, bool isLast)
{
	printer.InsertIndent();
	os << "\"" << name << "\": [";
	if (list.empty())
...
template<typename T>
void Print(std::ostringstream& os, ASTPrinter& printer, const char* name, T member, bool isLast)
{
	printer.InsertIndent();
	os << "\"" << name << "\": ";
	os << member;
...
template<>
void Print<IPLString>(std::ostringstream& os, ASTPrinter& printer, const char* name, IPLString member, bool isLast)
{
	printer.InsertIndent();
...
std::ostringstream& operator<<(std::ostringstream& os, const TokenType& t)
{
	switch (t)
	{
...
#define GENERATE_AST_PRINTER_FUNCTION(ClassName, MEMBERS_ITERATOR)           \
	...
	{                                                                        \
		...
		if(currentVisitCount == membersCount)								 \
		{																	 \
			os << "\n";														 \
		}																	 \
